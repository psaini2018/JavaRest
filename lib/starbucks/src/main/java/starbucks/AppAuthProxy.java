...
public class AppAuthProxy implements IApp, IPinAuthObserver {
     ...
     * @param status Auth even is success or failure
     */
    public void authEvent(Boolean status) {
        this.authenticated = status;
        Cards.getInstance().reInit();

        ((IKeyPadSubject) kp).removeObserver(pc);
        ((IKeyPadSubject) kp).removeObserver(pm);
        ((IKeyPadSubject) kp).attach(pc);
        ((IKeyPadSubject) kp).attach(pm);

        ps.clearInputs(status);
    }
}
